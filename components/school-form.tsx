...
export function SchoolForm() {
  ...
  // Filter indices based on search matching id, name, or demand
  const filteredIndices = fields.map((_, idx) => idx).filter((idx) => {
    ...
    return (
      ...
      s.demand?.toLowerCase().includes(q)
    );
  });
    ...
    <div>
      ...
      {fields.length > 0 && (
        <div className="relative mb-3">
          ...
          <Input
            type="text"
            placeholder="Search by name, ID, or demand subject..."
            value={search}
      ...
      <div className="grid gap-3">
            ...
            <div key={field.id} className="rounded-xl border border-line bg-card p-3.5">
              <div className="grid grid-cols-2 gap-2.5">
                <label className="grid gap-1 text-xs text-muted font-medium">
                  School ID
                  <Input {...register(`schools.${idx}.id`, { required: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
                <label className="grid gap-1 text-xs text-muted font-medium">
                  Name
                  <Input {...register(`schools.${idx}.name`, { required: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
                <label className="grid gap-1 text-xs text-muted font-medium">
                  Priority
                  <Input type="number" {...register(`schools.${idx}.priority`, { required: true, valueAsNumber: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
                <label className="grid gap-1 text-xs text-muted font-medium">
                  Latitude
                  <Input type="number" step="any" {...register(`schools.${idx}.lat`, { required: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
                <label className="grid gap-1 text-xs text-muted font-medium">
                  Longitude
                  <Input type="number" step="any" {...register(`schools.${idx}.lng`, { required: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
                <label className="grid gap-1 text-xs text-muted font-medium">
                  Demand (Math:12, English:8)
                  <Input {...register(`schools.${idx}.demand`, { required: true })} className="h-9 text-sm bg-background text-foreground" />
                </label>
              </div>
        ...
        {fields.length > 0 && filteredIndices.length === 0 && (
          <p className="text-sm text-muted text-center py-6">No schools match &ldquo;{search}&rdquo;</p>
        )}
        {fields.length === 0 && (
